...
name="base.high_channel.smallestdeep"
...
parser.add_argument( "--learning_rate", help="learning rate for this model", type=float, required=True )
...
if os.path.isfile( "../current." + name + ".h5" ):
    model = load_model( "../current." + name + ".h5" )
else:
...
for line in file_lines:
    ...
    #model.train_on_batch( x=[source_input,ray_input], y=output )
    model.fit( x=[source_input,ray_input], y=output, epochs=1, batch_size=32 )
    t2 = time.time()
