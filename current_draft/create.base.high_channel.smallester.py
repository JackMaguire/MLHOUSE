...
parser.add_argument( "--model", help="filename for output file", default="start.base.high_channel.smallester", required=False )
...
A = Conv2D( name="layerA", filters=15, kernel_size=(1,1), padding='valid', input_shape=pre.shape, data_format='channels_last', activation='relu', use_bias=True )( pre )
B = Conv2D( name="layerB", filters=5, kernel_size=(1,1), padding='valid', input_shape=A.shape, data_format='channels_last', activation='relu', use_bias=True )( A )
merge = tensorflow.keras.layers.concatenate( [B,in1up], name="merge", axis=-1 )

# Phase 2: FGHIJ
C = LocallyConnected2D( name="layerC", filters=7, kernel_size=(1,1), strides=(1,1), padding='valid', data_format='channels_last', activation='relu', use_bias=True )( merge )
D = LocallyConnected2D( name="layerD", filters=5, kernel_size=(1,1), strides=(1,1), padding='valid', data_format='channels_last', activation='relu', use_bias=True )( C )
...
print( Epool.shape )
F = LocallyConnected2D( name="layerF", filters=10, kernel_size=(6,4), strides=(4,3), padding='valid', data_format='channels_last', activation='relu', use_bias=True )( Epool )
print( F.shape )
