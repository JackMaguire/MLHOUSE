...
parser.add_argument( "--model", help="filename for output file", default="start.advanced2.5C.5L.10D.value", required=False )
...
# Phase 2: FGHIJ
F = LocallyConnected2D( name="layerF", filters=30, kernel_size=(3,2), padding='valid', input_shape=E.shape, data_format='channels_last', activation='relu', use_bias=True )( E )
G = LocallyConnected2D( name="layerG", filters=25, kernel_size=(3,2), padding='valid', input_shape=F.shape, data_format='channels_last', activation='relu', use_bias=True )( F )
H = LocallyConnected2D( name="layerH", filters=20, kernel_size=(3,2), padding='valid', input_shape=G.shape, data_format='channels_last', activation='relu', use_bias=True )( G )
I = LocallyConnected2D( name="layerI", filters=15, kernel_size=(3,2), padding='valid', input_shape=H.shape, data_format='channels_last', activation='relu', use_bias=True )( H )
J = LocallyConnected2D( name="layerJ", filters=10, kernel_size=(3,2), padding='valid', input_shape=I.shape, data_format='channels_last', activation='relu', use_bias=True )( I )
...
denseO = Dense( name="denseO", units=500, activation='relu' )( denseN )

denseP = Dense( name="denseP", units=250, activation='relu' )( denseO )
denseQ = Dense( name="denseQ", units=250, activation='relu' )( denseP )
denseR = Dense( name="denseR", units=250, activation='relu' )( denseQ )
denseS = Dense( name="denseS", units=250, activation='relu' )( denseR )
output = Dense( name="output", units=1, activation='linear' )( denseS )
