...
parser.add_argument( "--model", help="filename for output file", default="start.base3.branch", required=False )
...
in1dense4_size = 18 * 9 * 4
in1dense4 = Dense( name="in1dense4", units=in1dense4_size, activation=None )( in1dense3 )
in1merge = Reshape( target_shape=(18, 9, 4,) )( in1dense4 )#ALWAYS DO WIDTH, HEIGHT, CHANNELS
...
B1 = LeakyReLU()( B1 )
B4 = LocallyConnected2D( name="layerB4", filters=4, kernel_size=(2,3), strides=(2,2), padding='valid', data_format='channels_last', activation=None, use_bias=True )( B1 )

X = Conv2D( name="layerX", filters=8, kernel_size=(1,1), padding='valid',   data_format='channels_last', use_bias=True )( pre )
X = LeakyReLU()( X )
Y = LocallyConnected2D( name="layerY", filters=6, kernel_size=(1,1), padding='valid', data_format='channels_last', use_bias=True )( X )
Y = LeakyReLU()( Y )
Z = LocallyConnected2D( name="layerZ", filters=4, kernel_size=(2,3), strides=(2,2), padding='valid', data_format='channels_last', activation=None, use_bias=True )( Y )
