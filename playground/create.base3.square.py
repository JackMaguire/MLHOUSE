...
parser.add_argument( "--model", help="filename for output file", default="start.base3.square", required=False )
...
in1merge = Reshape( target_shape=(9, 9, 5,) )( in1dense4 )#ALWAYS DO WIDTH, HEIGHT, CHANNELS
...
B1 = LeakyReLU()( B1 )
B4 = LocallyConnected2D( name="layerB4", filters=5, kernel_size=(4,3), strides=(4,2), padding='valid', data_format='channels_last', activation=None, use_bias=True )( B1 )


C = Add()( [B4, in1merge] )
C = LeakyReLU( )( C )
E = LocallyConnected2D( name="layerE", filters=4, kernel_size=(3,3), strides=(3,3), padding='valid', data_format='channels_last', use_bias=True )( C )
E = LeakyReLU()( E )

'''
F = LocallyConnected2D( name="layerF", filters=3, kernel_size=(1,1), strides=(1,1), padding='valid', data_format='channels_last', use_bias=True )( E )
F = LeakyReLU()( F )
print( F.shape )
flat = Flatten( name="flat", data_format='channels_last' )( F )
'''

flat = Flatten( name="flat", data_format='channels_last' )( E )
